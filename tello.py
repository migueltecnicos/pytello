...
class Tello:
    ...
    def __init__(self, local_ip, local_port, imperial=False, command_timeout=.3, tello_ip='192.168.10.1',
        ...
        self.socket_telemetria = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # socket for receive telemetry
        self.socket_video = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # socket for receiving video stream
        self.tello_address = (tello_ip, tello_port)
        self.local_video_port = 11111  # port for receiving video stream
        self.local_telemetry_port = 8890
        self.last_height = 0
        self.socket.bind((local_ip, local_port))
        self.socket_telemetria.bind((local_ip, self.local_telemetry_port))
        ...
        # thread for receiving telemetry
        self.receive_telemetry_thread = threading.Thread(target=self._receive_telemetry_thread)
        self.receive_telemetry_thread.daemon = True
        self.receive_telemetry_thread.start()
    ...
    def _receive_telemetry_thread(self):
        """Lee los datos de telemetria del Tello.
        ...
        while True:
            try:
                response, ip = self.socket_telemetria.recvfrom(1000)
                print ip, "-->", response
                sys.stdout.flush()
            except socket.error as exc:
                print ("Caught exception socket.error : %s" % exc)
